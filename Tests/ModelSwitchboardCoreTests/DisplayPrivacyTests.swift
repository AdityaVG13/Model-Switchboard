...
@Test func displayPrivacyMasksHosts() {
    #expect(DisplayPrivacy.host("gpu.example.ts.net", hidden: true) == "••••")
    #expect(DisplayPrivacy.host("100.64.1.2", hidden: true) == "••••")
    #expect(DisplayPrivacy.host("user@gpu.local", hidden: true) == "••••")
...
@Test func displayPrivacyKeepsPortsAndURLShapes() {
    ...
    #expect(
        DisplayPrivacy.url("http://gpu.example.ts.net:8050/v1", hidden: true)
            == "http://••••:8050/v1"
    )
    #expect(
        DisplayPrivacy.url("http://100.64.1.2:8877", hidden: false)
            == "http://100.64.1.2:8877"
    )
...
@Test func displayPrivacyConnectionSummary() {
    #expect(
        DisplayPrivacy.connectionSummary("ssh user@gpu.local → 127.0.0.1:8877", hidden: true)
            == "•••• → 127.0.0.1:8877"
    )
    #expect(
        DisplayPrivacy.connectionSummary("ssh user@gpu.local → 127.0.0.1:8877", hidden: false)
            == "ssh user@gpu.local → 127.0.0.1:8877"
    )
    #expect(
        DisplayPrivacy.connectionSummary("http://host.example.ts.net:8877", hidden: true)
            == "http://••••:8877"
    )
