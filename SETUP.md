...
| Pair | Paste the installer's or `link`'s `modelswitchboard-gateway://…` code into **Settings → Remote Gateways → Add Remote Gateway**. It locally prefills editable user/host/port fields; no pairing service is involved. |
